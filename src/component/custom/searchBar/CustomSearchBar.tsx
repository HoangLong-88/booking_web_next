'use client'
import { DateBarRangePicker, DateBarSinglePicker } from '@/component/ui/DateBar';
import { KeySearchBar} from '@/component/ui/SearchBar';
...
export const DuelDateSearchBar = ({ service }: ServiceSearchProps) => {
    const { setLocation, setCheckIn, setCheckOut, handleSearch } = searchLocationAndDuelDate(service ?? '');
    return (
...
export const SingleDateSearchBar = ({service}: ServiceSearchProps) => {
    const {setCheckDate, setLocation, handleSearch} = searchWithSingleDate(service ?? '');  
    return (
        <div className="relative left-1/2 -translate-x-1/2 bottom-[40px] z-20 w-[92%] md:w-[85%] max-w-6xl">
            <form className="bg-white rounded-xl shadow-2xl ring-1 ring-black/5 p-3 flex gap-3 items-center border-2 border-orange-300"
                onSubmit={(e) => { e.preventDefault(); handleSearch(); }}>
